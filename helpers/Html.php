...
/**
 * Class Html
 * @package pvsaintpe\search\helpers
 */
class Html extends BaseHtml
{
    ...
    public static function createButton($text = null, array $options = [])
    {
        ...
        // icon
        $icon = static::glyphIconPlus();
        if (array_key_exists('icon', $options)) {
            $icon = static::glyphIcon($options['icon']);
            unset($options['icon']);
        }
        // css
        $css = static::addCssClass($options, ['btn', 'btn-success', 'btn-sm']);

        $options['data-pjax'] = '0';
        return static::a($icon . ' ' . $text, Url::toRoute($url), $options);
    }
     ...
     * @return string
     */
    public static function rewriteMiniButton(BaseActiveRecord $model, array $options = [])
    {
        if ($model->hasAttribute('rewrite')) {
            $enabled = $model->getAttribute('rewrite');
            // url
            $url = 'rewrite';
            if (array_key_exists('url', $options)) {
                $url = $options['url'];
            ...
            $options[$methodKey] = $method;
            return static::a(
                $model->getAttribute('rewrite') ? Yii::t('buttons', 'Выкл') : Yii::t('buttons', 'Вкл'),
                array_merge((array)$url, $model->getPrimaryKey(true)),
                $options
            );
        } else {
     ...
     * @return string
     */
    public static function replaceMiniButton(BaseActiveRecord $model, array $options = [])
    {
        if ($model->hasAttribute('replace')) {
            $enabled = $model->getAttribute('replace');
            // url
            $url = 'replace';
            if (array_key_exists('url', $options)) {
                $url = $options['url'];
            ...
            $options[$methodKey] = $method;
            return static::a(
                $model->getAttribute('replace') ? Yii::t('buttons', 'Выкл') : Yii::t('buttons', 'Вкл'),
                array_merge((array)$url, $model->getPrimaryKey(true)),
                $options
            );
        } else {
     ...
     * @return string
     */
    public static function customButton($label, $class = 'btn-default', $submit = true, $options = [], $url = null)
    {
        if ($submit) {
            return static::submitButton(
                Yii::t('buttons', $label),
    ...
    public static function indexViewButtons($permissionPrefix, $createButtonText = null)
    {
        ...
        if (Yii::$app->getUser()->can($permissionPrefix . 'create')) {
            $buttons[] = Html::createButton($createButtonText, ['url' => '/' . $permissionPrefix . 'create']);
        }
        return $buttons;
    }
    ...
    public static function switchDeleteItem(BaseActiveRecord $model, array $options = [])
    {
        if ($model->hasAttribute('deleted')) {
            ...
            return [
                'label' => (!$deleted
                        ? Yii::t('buttons', 'Удалить')
                        : Yii::t('buttons', 'Восстановить')
                    ),
                'url' => array_merge((array)$url, $model->getPrimaryKey(true)),
                'linkOptions' => $options,
                'encode' => false
